...
@section('title', 'Editar Precio Global')
        ...
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0">Modifica la configuración del precio global</p>
                </div>
                <div>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('superadmin.pricing.update', $pricing) }}" method="POST" id="pricingForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('price_type') is-invalid @enderror" 
                                    ...
                                    <option value="daily" {{ old('price_type', $pricing->price_type) === 'daily' ? 'selected' : '' }}>
                                        Diario (por día)
                                    </option>
                                    <option value="nightly" {{ old('price_type', $pricing->price_type) === 'nightly' ? 'selected' : '' }}>
                                        Nocturno (por noche)
                                    </option>
                                </select>
                                @error('price_type')
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control @error('base_price') is-invalid @enderror" 
                                           id="base_price" name="base_price" value="{{ old('base_price', $pricing->base_price) }}" 
                                           step="0.01" min="0" placeholder="0.00" required>
                                </div>
                                <div class="form-text">Precio base por {{ old('price_type', $pricing->price_type) === 'nightly' ? 'noche' : 'día' }}</div>
                                @error('base_price')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" 
                                          ...
                                          placeholder="Descripción opcional del precio">{{ old('description', $pricing->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="card-header">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="has_discount" name="has_discount" 
                                           value="1" {{ old('has_discount', $pricing->has_discount) ? 'checked' : '' }}>
                                    <label class="form-check-label fw-bold" for="has_discount">
                                        Aplicar Descuento
                                    </label>
                                </div>
                            </div>
                            <div class="card-body" id="discount_section" style="display: {{ old('has_discount', $pricing->has_discount) ? 'block' : 'none' }};">
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select @error('discount_type') is-invalid @enderror" 
                                            ...
                                            <option value="">Seleccionar tipo</option>
                                            <option value="percentage" {{ old('discount_type', $pricing->discount_type) === 'percentage' ? 'selected' : '' }}>
                                                Porcentaje (%)
                                            </option>
                                            <option value="fixed" {{ old('discount_type', $pricing->discount_type) === 'fixed' ? 'selected' : '' }}>
                                                Monto Fijo ($)
                                            </option>
                                        </select>
                                        @error('discount_type')
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="input-group">
                                            <input type="number" class="form-control @error('discount_percentage') is-invalid @enderror" 
                                                   ...
                                                   value="{{ old('discount_percentage', $pricing->discount_percentage) }}" 
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <small class="text-muted">
                                                    <strong>Creado:</strong> {{ $pricing->created_at->format('d/m/Y H:i') }}<br>
                                                    <strong>Por:</strong> {{ $pricing->creator->name ?? 'Sistema' }}
                                                </small>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="text-muted">
                                                    <strong>Última actualización:</strong> {{ $pricing->updated_at->format('d/m/Y H:i') }}<br>
                                                    <strong>Por:</strong> {{ $pricing->updater->name ?? 'Sistema' }}
                                                </small>
                                            </div>
                                        </div>
                                    </div>
