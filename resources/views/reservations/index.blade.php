    ...
    @if($reservations->count() > 0)
        ...
        <div class="reservations-grid">
            ...
            @if($reservations->where('status', 'pending')->count() > 0)
                <div class="pending-reservations-section mb-5">
                    <div class="section-header mb-4">
                        ...
                        <p class="section-subtitle text-muted">
                            Estas reservas están esperando la aprobación del administrador
                        </p>
                    </div>
                    <div class="row row-cols-1 row-cols-lg-2 g-4">
                        @foreach($reservations->where('status', 'pending') as $reservation)
                            @include('reservations.partials.reservation-card', ['reservation' => $reservation, 'highlight' => true])
                        @endforeach
                    </div>
                </div>
            @endif

            <!-- Sección de Otras Reservas -->
            @if($reservations->where('status', '!=', 'pending')->count() > 0)
                <div class="other-reservations-section">
                    <div class="section-header mb-4">
                        ...
                        <p class="section-subtitle text-muted">
                            Reservas aprobadas, completadas y canceladas
                        </p>
                    </div>
                    <div class="row row-cols-1 row-cols-lg-2 g-4">
                        @foreach($reservations->where('status', '!=', 'pending') as $reservation)
                            @include('reservations.partials.reservation-card', ['reservation' => $reservation, 'highlight' => false])
                        @endforeach
                    </div>
                </div>
            @endif
...
.pending-reservations-section::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
    right: 0;
    bottom: 0;
    background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><pattern id="pending-pattern" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="10" cy="10" r="1" fill="rgba(255,193,7,0.1)"/></pattern></defs><rect width="100" height="100" fill="url(%23pending-pattern)"/></svg>');
    opacity: 0.3;
}
...
/* Tarjetas destacadas (pendientes) */
.reservation-card-modern.highlighted {
    border: 2px solid #ffc107;
    box-shadow: 0 8px 25px rgba(255, 193, 7, 0.3);
    position: relative;
}

.reservation-card-modern.highlighted::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
...
.reservation-card-modern.highlighted .reservation-header {
    background: linear-gradient(135deg, #fff3cd 0%, #ffeaa7 100%);
    border-bottom-color: #ffc107;
}

.reservation-card-modern.highlighted .reservation-status .badge {
...
// Cargar datos pendientes al cargar la página (ya no necesario)
document.addEventListener('DOMContentLoaded', function() {
    // Ya no necesitamos cargar datos pendientes
    // La reserva se crea directamente desde la página de propiedad
});
</script>
@endpush
